...
\begin{itemize}
  ...
  \item Migrated every ingestion source off paid platforms to self-hosted Airbyte on EKS, cutting
        ETL licence spend \cvmetric{94\%}, around \$60k per year
  ...
  \item Profiled the production MySQL workload and removed \cvmetric{8\%} of its query volume,
        including an N+1 lookup cached per item and an unindexed join scanning around 40,000 rows a
        second to return 1,500
...
\begin{itemize}
  ...
  \item Sole maintainer of the Airflow platform on EKS: migrated \cvmetric{12} DAGs off v1, took
        it through every upgrade to v3.3, and cut image builds from 7 minutes to under 30 seconds
        by no longer compiling the image from source on every run
\end{itemize}
